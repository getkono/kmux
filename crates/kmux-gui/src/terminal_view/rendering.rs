...
use iced::{
    Color as IcedColor, Font, Pixels, Point as IcedPoint, Rectangle, Size,
    font::{Style, Weight},
    mouse,
    widget::canvas::{self},
};
...
use kmux_client::grid::{CELL_HEIGHT, CELL_WIDTH};

use crate::app::Message;

use super::FONT_SIZE;
use super::cell::{default_bg, draw_cell};
use super::hud::{draw_hud, draw_scroll_indicator};
use super::selection::draw_selection_overlay;
use super::{CanvasState, GridView};
...
impl<'a> canvas::Program<Message> for GridView<'a> {
    ...
    fn update(
        ...
        use super::geometry::pixel_to_grid_pos;
        use kmux_client::grid::{MULTI_CLICK_TIMEOUT_MS, SelectionMode};
            ...
            && state.mouse_down
        {
            if let Some(pos) = cursor.position_in(bounds) {
                let grid_pos = super::geometry::pixel_to_grid_pos(
                    pos.x,
                    pos.y,
                    self.cols,
                    self.rows,
