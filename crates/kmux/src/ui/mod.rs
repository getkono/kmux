...
fn render_terminal(f: &mut Frame, app: &mut App) {
    ...
    match &mode_snap {
        ...
        // Defensive guard: if the connection is already live, the mode is
        // stale and the overlay would spuriously cover the live grid. The
        ...
        Mode::Connecting { target_display } if !app.mgr.connection_state().is_live() => {
