...
impl TimeoutEnforcer {
    ...
    pub fn check(&self, started_at: Instant) -> Option<SmuxError> {
        if let Some(wall) = self.wall_clock
            && started_at.elapsed() >= wall
        {
            return Some(SmuxError::Timeout);
        }
        if let Some(idle) = self.idle {
            let elapsed = self.last_activity.elapsed();
            if elapsed >= idle {
                return Some(SmuxError::IdleTimeout {
    ...
    ) -> watch::Receiver<Option<SmuxError>> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                    ...
                    && started.elapsed() >= wall
                {
                    ...
                    let _ = tx.send(Some(SmuxError::Timeout));
                    break;
                }
